...
(() => {
  ...
  async function getSettings() {
    const { settings } = await import_webextension_polyfill.default.storage.local.get("settings");
    return settings || DEFAULT_SETTINGS;
  }
  async function getProducts() {
    const { trackedProducts } = await import_webextension_polyfill.default.storage.local.get("trackedProducts");
    return trackedProducts || [];
  }
  async function saveProducts(products) {
    await import_webextension_polyfill.default.storage.local.set({ trackedProducts: products });
  }

  // src/background.ts
  var RAPIDAPI_HOST = "real-time-amazon-data.p.rapidapi.com";
  var DEFAULT_INTERVAL_MINUTES = 180;
  var ALARM_NAME = "priceCheckAlarm";
  ...
  async function fetchProductDetails(asin, country, apiKey) {
    ...
    const title = data.product_title || data.title || "Unknown product";
    const image = data.product_photo || data.product_image || data.image || "";
  ...
  async function removeProduct(asin, country) {
    const products = await getProducts();
    await saveProducts(products.filter((p) => !(p.asin === asin && p.country === country)));
  }
  async function setTargetPrice(asin, country, targetPrice) {
    const products = await getProducts();
  ...
  async function checkOneProduct(product, apiKey) {
    try {
      ...
      const targetReached = product.targetPrice !== null && product.targetPrice !== void 0 && details.price <= product.targetPrice;
      if (priceDropped) {
        await import_webextension_polyfill2.default.notifications.create(`price-drop-${product.asin}-${Date.now()}`, {
          type: "basic",
          iconUrl: "icons/icon128.png",
          title: "Price Drop! \u{1F4C9}",
...
${product.currency}${product.lastPrice} \u2192 ${details.currency}${details.price}`,
      ...
      if (targetReached) {
        await import_webextension_polyfill2.default.notifications.create(`target-reached-${product.asin}-${Date.now()}`, {
          type: "basic",
          iconUrl: "icons/icon128.png",
          title: "Target Price Reached! \u{1F3AF}",
      ...
      product.lastError = String(err.message || err);
      product.lastCheckedAt = Date.now();
    }
    return product;
  }
  ...
  async function scheduleAlarm() {
    ...
    await import_webextension_polyfill2.default.alarms.create(ALARM_NAME, { periodInMinutes: minutes, delayInMinutes: 1 });
  }
  import_webextension_polyfill2.default.runtime.onInstalled.addListener(() => {
    scheduleAlarm();
  });
  import_webextension_polyfill2.default.alarms.onAlarm.addListener((alarm) => {
    if (alarm.name === ALARM_NAME) {
      checkAllProducts();
    }
  });
  import_webextension_polyfill2.default.runtime.onMessage.addListener((raw) => {
    const message = raw;
    return (async () => {
      try {
        switch (message.type) {
          ...
          default: {
            ...
            return { ok: false, error: `Unknown message type: ${String(_exhaustive?.type)}` };
          }
        }
      } catch (err) {
