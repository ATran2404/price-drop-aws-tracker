...
(() => {
  ...
  var noKeyWarningEl = document.getElementById("noKeyWarning");
  var checkAllBtn = document.getElementById("checkAllBtn");
  var settingsBtn = document.getElementById("settingsBtn");
  var openOptionsLink = document.getElementById("openOptionsLink");
  ...
  function renderProduct(p) {
    ...
    <div class="product-info">
      ...
      <div class="price-row">
        ...
        ${p.initialPrice !== p.lastPrice ? `<span class="original-price">${p.currency || ""}${p.initialPrice}</span>` : ""}
      </div>
      <div class="meta-row">
        Lowest: ${p.currency || ""}${p.lowestPrice} \xB7 Checked ${timeAgo(p.lastCheckedAt)}
  ...
  `;
    item.querySelector(".open-btn").addEventListener("click", () => {
      import_webextension_polyfill.default.tabs.create({ url: p.url });
    });
  ...
  async function loadProducts() {
    const { trackedProducts, settings } = await import_webextension_polyfill.default.storage.local.get(["trackedProducts", "settings"]);
    ...
    products.slice().sort((a, b) => b.addedAt - a.addedAt).forEach((p) => productListEl.appendChild(renderProduct(p)));
  }
